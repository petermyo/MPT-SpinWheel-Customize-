...
import { Settings, X, Sparkles, TrendingUp, Trophy, PartyPopper } from 'lucide-react';
import confetti from 'canvas-confetti';

const CLAP_SOUND_URL = 'https://actions.google.com/sounds/v1/human_voices/applause_moderate.ogg';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    clapAudioRef.current = new Audio(CLAP_SOUND_URL);
  }, []);
  ...
  const handleWheelResult = async (winningSlice: SpinSlice) => {
    ...
    // Trigger Confetti for Big Wins (Amount >= 5000)
    if (parseInt(winningSlice.amount) >= 5000) {
        triggerConfetti();
    }
    ...
    try {
      ...
      const prompt = winningSlice.amount === '0' 
        ...
        : `An MPT user just won "${winningSlice.title}" (Value: ${winningSlice.amount}). Write a short, exciting congratulatory 1-sentence message.`;
      ...
      setWinnerMessage(winningSlice.amount === '0' 
        ? "Better luck next time! Keep spinning for more chances!" 
        : `Fantastic! You've just unlocked ${winningSlice.title}! 🥳`);
    }
  };
    ...
    <div className="min-h-screen bg-mpt-blue selection:bg-mpt-yellow selection:text-blue-900 text-white pb-10 overflow-x-hidden">
        ...
        <div className="lg:col-span-3 space-y-8 sticky top-32">
            <div className="w-full bg-white/10 rounded-[2.5rem] p-10 border-2 border-white/10 backdrop-blur-xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)]">
                ...
                <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="bg-gradient-to-br from-white/10 to-transparent p-6 rounded-3xl border border-white/10">
                        <div className="text-[10px] text-mpt-yellow/60 uppercase font-black mb-1 tracking-widest">Jackpots Today</div>
                        <div className="text-4xl font-black text-mpt-yellow tabular-nums tracking-tighter">
                            {history.filter(h => parseInt(h.amount) >= 5000).length}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white/95 backdrop-blur-md rounded-[2.5rem] p-10 text-gray-800 shadow-2xl border-4 border-white/50">
                ...
                <ul className="space-y-6">
                    <li className="flex gap-4 items-start">
                        ...
                        <p className="text-sm font-bold text-gray-600 leading-snug">Tap <span className="text-blue-900">PLAY</span> to start your fortune spin.</p>
                    </li>
                    <li className="flex gap-4 items-start">
                        ...
                        <p className="text-sm font-bold text-gray-600 leading-snug">Wait for pointer to select your reward.</p>
                    </li>
                    <li className="flex gap-4 items-start">
                        ...
                        <p className="text-sm font-bold text-gray-600 leading-snug">Big amounts unlock the <span className="text-mpt-blue">Confetti Rain!</span></p>
                    </li>
                </ul>
            </div>
        </div>
      ...
      {showWinnerPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            <div className="absolute inset-0 bg-mpt-blue/80 backdrop-blur-2xl animate-in fade-in duration-500" onClick={() => setShowWinnerPopup(false)} />
            <div className="relative bg-white w-full max-w-md rounded-[3rem] p-10 text-center shadow-[0_0_100px_rgba(255,209,0,0.5)] border-8 border-mpt-yellow animate-in zoom-in slide-in-from-bottom-20 duration-500">
                <div className="absolute -top-16 left-1/2 -translate-x-1/2 bg-mpt-yellow w-32 h-32 rounded-full border-8 border-white flex items-center justify-center shadow-2xl animate-bounce-slow">
                    <Trophy size={60} className="text-blue-900" />
                </div>
                <div className="mt-16 mb-8">
                    <h2 className="text-blue-900 font-black text-5xl tracking-tighter uppercase mb-2">WOW!</h2>
                    <div className="text-6xl mb-4 transform scale-125">{lastWin?.icon}</div>
                    <p className="text-gray-500 font-black text-xs uppercase tracking-[0.3em] mb-4">You just won</p>
                    <div className="bg-mpt-blue/5 py-4 px-8 rounded-3xl inline-block border-2 border-mpt-blue/10">
                        <span className="text-mpt-blue font-black text-4xl italic tracking-tighter">{lastWin?.title}</span>
                    </div>
                </div>
                <p className="text-gray-700 font-bold text-lg leading-relaxed mb-10 px-4">
                    {winnerMessage}
                </p>
                <button 
                    onClick={() => setShowWinnerPopup(false)}
                    className="w-full py-5 bg-mpt-yellow hover:bg-blue-900 hover:text-white text-blue-900 rounded-2xl font-black text-2xl uppercase tracking-tighter transition-all shadow-xl active:scale-95"
                >
                    AWESOME!
                </button>
            </div>
