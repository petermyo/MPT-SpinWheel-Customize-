...
const TICK_SOUND_URL = 'https://www.soundjay.com/buttons/button-29.mp3';
...
const SpinWheel: React.FC<SpinWheelProps> = ({ slices, onResult, isSpinning, setIsSpinning }) => {
  ...
  const handleSpin = () => {
    ...
    const animate = (now: number) => {
      ...
      // Pointer is at 1.5 PI. We find which slice is at 1.5 PI.
      // Pointer Angle - currentRotation = sliceAngle * sliceIndex
  ...
  return (
    <div className="relative flex flex-col items-center">
      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-8 z-30 pointer-events-none">
        <svg width="46" height="56" viewBox="0 0 46 56" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-[0_4px_12px_rgba(0,0,0,0.6)]">
      ...
      <div className="bg-white/5 p-8 rounded-full border-[14px] border-mpt-yellow shadow-[0_0_80px_rgba(255,209,0,0.5)] backdrop-blur-md relative group">
        <div className="absolute inset-2 rounded-full border-2 border-dashed border-white/30 animate-[spin_40s_linear_infinite]"></div>
        
        <canvas 
          ref={canvasRef} 
          width={460} 
          height={460} 
          className="max-w-full h-auto drop-shadow-[0_20px_40px_rgba(0,0,0,0.5)]"
        />
      </div>
      ...
      <button
        ...
        className={`mt-12 px-20 py-6 rounded-full font-black text-4xl uppercase tracking-tighter shadow-2xl transition-all active:scale-95 border-b-[10px] relative overflow-hidden ${
          isSpinning 
            ...
            : 'bg-mpt-yellow border-yellow-600 text-blue-900 hover:bg-white hover:text-mpt-blue hover:-translate-y-2 hover:shadow-yellow-400/80'
        }`}
      >
        <span className="relative z-10">{isSpinning ? 'SPINNING...' : 'TAP TO PLAY'}</span>
        {!isSpinning && <div className="absolute inset-0 bg-white/30 -translate-x-full hover:translate-x-full transition-transform duration-1000 ease-in-out"></div>}
      </button>
      
      <p className="mt-4 text-white/40 text-xs font-black tracking-[0.2em] uppercase">
        {isSpinning ? 'BRACE FOR LUCK!' : 'WIN EXCITING DATA & PRIZES'}
      </p>
    </div>
  );
};
